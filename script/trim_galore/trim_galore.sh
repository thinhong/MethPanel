...
if [[ "$isSingleEnd" -eq "1" ]]; then
	# trim single end
	"$trim_galore" -q $BASE_QUALITY --illumina --length $READ_LENGTH -o "$O" --clip_R1 1 --three_prime_clip_R1 1 --gzip --fastqc --fastqc_args "--outdir $O" --trim-n "$R1"
	# rename after timmed
	for i in `find "$O" -name "*.fq.gz"`; do mv "$i" "${i/_R1_trimmed.fq.gz/_trimmed_R1.fastq.gz}"; done
else
	# trim paired end
	$trim_galore -q $BASE_QUALITY --illumina --length $READ_LENGTH -o "$O" --clip_R1 1 --clip_R2 1 --three_prime_clip_R1 1 --three_prime_clip_R2 1 --gzip --fastqc --fastqc_args "--outdir $O" --trim-n --paired "$R1" "$R2"
	# rename after timmed
